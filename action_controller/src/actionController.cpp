...
private:
	...
	ros::Publisher trajectory_pub;
  	...
  	mav_msgs::CommandTrajectory desired_wp0;

  	tf::Quaternion q;
  	tf::Quaternion q0;
  	double des_roll, des_pitch, des_yaw;
  	double des_roll0, des_pitch0, des_yaw0;
...
//Nishat 3.14159265
  				if(k<(toExecute.points.size()-1)){
					geometry_msgs::Transform_<std::allocator<void> > point0=toExecute.points[k+1].transforms[0];
					tf:quaternionMsgToTF(point0.rotation, q0);
					desired_wp0.position.x = point0.translation.x;
  					desired_wp0.position.y = point0.translation.y;
					desired_wp0.position.z = point0.translation.z;
  					tf::Matrix3x3(q0).getRPY(des_roll0, des_pitch0, des_yaw0); 
  			    } 
//Nishat  		
				if(k<(toExecute.points.size()-1)) {
  					desired_wp.yaw = atan2((desired_wp0.position.y-desired_wp.position.y),(desired_wp0.position.x-desired_wp.position.x));
  				}else{ 
